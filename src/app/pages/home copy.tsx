...
import { Add, Check, Close, PlayArrow, Stop, Timer } from '@mui/icons-material';
...
export default function HomePage() {
  ...
  const [cronometros, setCronometros] = useState<{
    [key: number]: number | NodeJS.Timeout | null;
  }>({});

  const [cronometroInicios, setCronometroInicios] = useState<{
    [key: number]: number;
  }>({});

  const [cronometroAtivo, setCronometroAtivo] = useState<number | null>(null);

  const handleCronometro = (subtarefaId: number) => {
    if (cronometros[subtarefaId]) {
      // Parar o cronômetro
      ...
      setCronometroAtivo(subtarefaId);
      setCronometroInicios({ ...cronometroInicios, [subtarefaId]: inicio });
